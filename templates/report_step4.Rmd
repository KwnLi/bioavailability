...
simTable <- data.frame(`Model input` = c(
  paste0("Measured EPC (mg bioavailable ", params$step4_output$AsPb, " per kg)"),
  "Measured EPC (% above/below the AL)",
  paste0("CoV in total ", params$step4_output$AsPb, " across the DU "),
  "CoV in % RBA across the DU",
  "Estimated mean % RBA"),
  `Value inferred post-sampling based on sampling results` =
    c(params$step4_output$step3$meas.ba,
      100*params$step4_output$step3$meas.frcAct,
      params$step4_output$step3$coeV.tot,
      params$step4_output$step3$coeV.rba,
      params$step4_output$step3$mn.rba),
  check.names = FALSE
  )
...
simIntTable <- data.frame(
  `Intermediete values used to derive updated model inputs` =
    c(paste0("S.D. in total ", params$step4_output$AsPb, " across composites**"),
      "S.D. in % RBA across composites*"),
  `Value inferred post-sampling based on sampling results` =
    c(params$step4_output$step3$sd.tot,
      params$step4_output$step3$sd.rba),
  check.names = FALSE
  )
...
```{r plot, fig.width = 6, fig.height=3.5}

plotData <- step4_plot(params$step4_output, sm.text = 7, lg.text = 8, strp.text = 10)

plotData$outplot

```

```{r text1, results='asis', warning = FALSE}

text1 <- gsub("<b>|</b>","**", plotData$accuracyText)
...
text2 <- gsub("<b>|</b>","**", plotData$precisionText)
...
`**` S.D. observed across X composites converted to CoV in total `r params$step4_output$AsPb` or % RBA using the following equation: 
