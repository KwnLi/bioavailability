  ...
  sidebarPanel(
    tabsetPanel(
      tabPanel(
        ...
        # composite total
        radioButtons("comp_tot", "Sample aggregation (tot. conc.)", choices = c(Discrete = FALSE, Composite = TRUE)),
        conditionalPanel(
          condition = "input.comp_tot == 'TRUE'",
          numericInput("tot.incr", label = div(style = "font-weight: normal; font-style: italic", "*Increments per composite total sample:"), 1, min = 2)),
        
        numericInput("IVBA.n", "# of samples to be analyzed for IVBA", 3),
        
        # composite IVBA
        radioButtons("comp_ivba", "Sample aggregation (IVBA)", choices = c(Discrete = FALSE, Composite = TRUE)),
        conditionalPanel(
          condition = "input.comp_ivba == 'TRUE'",
          numericInput("ivba.incr", label = div(style = "font-weight: normal; font-style: italic", "*Increments per composite IVBA sample:"), 1, min = 2)),
