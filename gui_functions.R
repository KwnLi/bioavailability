...
step1a_plot <- function(step1a.output){
  ...
  outputText <- paste(
    ...
    ifelse(measured.EPC < action.level, "less than", "greater than"), 
    " the action level, matching the conclusions of the measured EPC.", sep = ""
  )
...
step2_text_t1 <- function(step2_t1, error_threshold){
  ...
  return(paste0("<b>Type 1 error:</b> the simulation estimates that", message.type1))
}
...
step2_text_t2 <- function(step2_t2, error_threshold){
  ...
  return(paste0("<b>Type 2 error:</b> the simulation estimates that", message.type2))
}
